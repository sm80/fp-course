...
instance Applicative List where
  ...
  -- lfab <*> la = flatMap (\fab -> map fab la) lfab
  lfab <*> la = flatMap (`map` la) lfab
...
-- sequence lka = foldRight f (pure Nil) lka
--   where f ka kla = lift2 (:.) ka kla
...
-- filtering fakb la = foldRight f (pure Nil) la
--   where
