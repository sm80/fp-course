...
map fab = foldRight (\a lb -> fab a :. lb) Nil
...
filter fab = foldRight f Nil
  where f a lb = if fab a then a :. lb else lb
...
-- seqOptional loa = foldRight f (Full Nil) loa
--   where
--     f Empty _ = Empty
--     f _ Empty = Empty
--     f (Full a) (Full la) = Full (a :. la)
...
-- find fab la = foldRight f Empty la
...
lengthGT4 = lengthGT 4
  where
    ...
    lengthGT 0 _ = True
    lengthGT n (_ :. la) = lengthGT (n-1) la
...
produce faa a = a :. map faa (produce faa a)
